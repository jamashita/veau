...
export class StatsController {
  ...
  public list(@Param('page') pg: number, @Res() res: Response): Promise<Response> {
    return Page.of(pg).map<Response, PageError | StatsOutlinesError | DataSourceError>(
      (page: Page) => {
        return this.statsInteractor.findByVeauAccountID(res.locals.account.getVeauAccountID(), page).map<Response, StatsOutlinesError | DataSourceError>((outlines: JSONable) => {
  ...
  public refer(@Param('statsID') id: string, @Res() res: Response): Promise<Response> {
    return StatsID.ofString(id).map<Response, StatsIDError | StatsError | NoSuchElementError | DataSourceError>(
      (statsID: StatsID) => {
        return this.statsInteractor.findByStatsID(statsID).map<Response, StatsError | NoSuchElementError | DataSourceError>((stats: JSONable) => {
  ...
  public register(@Body({required: true}) body: PlainObject, @Res() res: Response): Promise<Response> {
    return Stats.ofObject(body).map<Response, StatsError | DataSourceError>((stats: Stats) => {
      return this.statsInteractor.save(stats, res.locals.account.getVeauAccountID()).map<Response, DataSourceError>(() => {
        return res.sendStatus(CREATED);
      });
    }, DataSourceError).recover<Response, Error>((err: StatsError | DataSourceError) => {
      logger.warn(err);
