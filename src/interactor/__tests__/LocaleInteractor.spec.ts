...
describe('LocaleInteractor', () => {
  describe('container', () => {
    it('must be a singleton', () => {
      ...
      const localeInteractor1: LocaleInteractor = cask.get<LocaleInteractor>(Type.LocaleInteractor);
      const localeInteractor2: LocaleInteractor = cask.get<LocaleInteractor>(Type.LocaleInteractor);
  ...
  describe('all', () => {
    it('normal case', async () => {
      ...
      const languageCaskQuery: MockLanguageQuery = new MockLanguageQuery();

      const stub1: SinonStub = sinon.stub();

      languageCaskQuery.all = stub1;
      stub1.returns(Superposition.alive<Languages, DataSourceError>(languages, DataSourceError));

      const regionCaskQuery: MockRegionQuery = new MockRegionQuery();
      const stub2: SinonStub = sinon.stub();

      regionCaskQuery.all = stub2;
