...
import { AccountMySQLQuery } from '../AccountMySQLQuery';

describe('AccountMySQLQuery', () => {
  describe('container', () => {
    it('must be a singleton', () => {
      expect.assertions(2);

      const accountQuery1: AccountMySQLQuery = cask.get<AccountMySQLQuery>(Type.AccountMySQLQuery);
      const accountQuery2: AccountMySQLQuery = cask.get<AccountMySQLQuery>(Type.AccountMySQLQuery);

      expect(accountQuery1).toBeInstanceOf(AccountMySQLQuery);
  ...
  describe('findByAccount', () => {
    it('normal case', async () => {
      ...
      const accountQuery: AccountMySQLQuery = new AccountMySQLQuery(mysql);
