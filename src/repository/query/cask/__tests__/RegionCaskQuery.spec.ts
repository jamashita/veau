...
import { RegionCaskQuery } from '../RegionCaskQuery';

describe('RegionCaskQuery', () => {
  describe('container', () => {
    it('must be a singleton', () => {
      expect.assertions(2);

      const regionQuery1: RegionCaskQuery = cask.get<RegionCaskQuery>(Type.RegionCaskQuery);
      const regionQuery2: RegionCaskQuery = cask.get<RegionCaskQuery>(Type.RegionCaskQuery);

      expect(regionQuery1).toBeInstanceOf(RegionCaskQuery);
  ...
  describe('all', () => {
    it('regionRedisQuery returns Alive', async () => {
      ...
      const regionQuery: RegionCaskQuery = new RegionCaskQuery(regionMySQLQuery, regionRedisQuery, regionRedisCommand);
