...
import { LanguageBinQuery } from '../LanguageBinQuery';

describe('LanguageBinQuery', () => {
  describe('container', () => {
    it('must be a singleton', () => {
      expect.assertions(2);

      const languageQuery1: LanguageBinQuery = bin.get<LanguageBinQuery>(Type.LanguageBinQuery);
      const languageQuery2: LanguageBinQuery = bin.get<LanguageBinQuery>(Type.LanguageBinQuery);

      expect(languageQuery1).toBeInstanceOf(LanguageBinQuery);
  ...
  describe('all', () => {
    it('normal case', async () => {
      ...
      const languageQuery: LanguageBinQuery = new LanguageBinQuery(localeBinQuery);
  ...
  describe('find', () => {
    ...
    it('localeQuery.all returns Dead, LanguageError', async () => {
      ...
      const languageQuery: LanguageBinQuery = new LanguageBinQuery(localeVaultQuery);
      const schrodinger: Schrodinger<Language, DataSourceError | LanguageError | NoSuchElementError> = await languageQuery.find(languageID).terminate();
