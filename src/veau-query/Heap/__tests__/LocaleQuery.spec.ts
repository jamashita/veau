...
describe('LocaleQuery', () => {
  ...
  describe('findByISO639', () => {
    it('normal case', async () => {
      ...
      const localeQuery: LocaleQuery = new LocaleQuery(heap);
      const trial: Try<Language, NoSuchElementError | DataSourceError> = await localeQuery.findByISO639(ISO639.of('aa'));

      expect(trial.isSuccess()).toEqual(true);
      const language: Language = trial.get();
      expect(language.getLanguageID()).toEqual(locale.getLanguages().get(0).get().getLanguageID());
      expect(language.getName()).toEqual(locale.getLanguages().get(0).get().getName());
      expect(language.getEnglishName()).toEqual(locale.getLanguages().get(0).get().getEnglishName());
      expect(language.getISO639()).toEqual(locale.getLanguages().get(0).get().getISO639());
    });
    ...
    it('returns Failure when Heap throws HeapError', async () => {
      ...
      const localeQuery: LocaleQuery = new LocaleQuery(heap);
      const trial: Try<Language, NoSuchElementError | DataSourceError> = await localeQuery.findByISO639(ISO639.of('cc'));
    ...
    it('throws Error', async () => {
      ...
      const localeQuery: LocaleQuery = new LocaleQuery(heap);
      try {
        await localeQuery.findByISO639(ISO639.of('cc'));
        spy1();
      }
      catch (err) {
        spy2();
  ...
  describe('findByISO3166', () => {
    it('normal case', async () => {
      ...
      const localeQuery: LocaleQuery = new LocaleQuery(heap);
      const trial: Try<Region, NoSuchElementError | DataSourceError> = await localeQuery.findByISO3166(ISO3166.of('bb'));

      expect(trial.isSuccess()).toEqual(true);
      const region: Region = trial.get();
      expect(region.getRegionID()).toEqual(locale.getRegions().get(0).get().getRegionID());
      expect(region.getName()).toEqual(locale.getRegions().get(0).get().getName());
      expect(region.getISO3166()).toEqual(locale.getRegions().get(0).get().getISO3166());
    });
    ...
    it('returns Failure when Heap throws HeapError', async () => {
      ...
      const localeQuery: LocaleQuery = new LocaleQuery(heap);
      const trial: Try<Region, NoSuchElementError | DataSourceError> = await localeQuery.findByISO3166(ISO3166.of('cc'));
    ...
    it('throws Error', async () => {
      ...
      const localeQuery: LocaleQuery = new LocaleQuery(heap);
      try {
        await localeQuery.findByISO3166(ISO3166.of('cc'));
        spy1();
      }
      catch (err) {
        spy2();
